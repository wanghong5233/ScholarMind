...
Agent / LLM 应用工程是新领域，工程直觉还在沉淀中，少有人系统总结。把 ScholarMind / Doc Studio 这类项目踩过的坑抽象为可复用直觉，是有差异化价值的——既能服务自己未来项目，也能投技术博客 / 飞书获得反馈。
...
playbook 的灵魂是"跨项目复用"。出现 `SM_*` env 或 `LlamaParse` 等业务名词，会让读者立刻判定"这是 ScholarMind 的事，跟我无关"，价值归零。

抽象替换测试（见 `examples.md`）是判别工具：替换后文章仍成立 → 合格；散架 → 这是 pitfall 不是 playbook。

## 为什么单向引用（pitfall → playbook，反之禁止）

- pitfall 项目内，路径具体（`/opt/apps/scholarmind` / `docker-compose.prod.yml`），生命周期数月
