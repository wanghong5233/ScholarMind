...
1.  **需求分析与接口定义 (Define the Contract)**
    *   **目标**: 明确功能的输入、输出和核心行为。
    *   **行动**: 在动手编码前，首先定义后端需要暴露的API接口（输入参数、返回数据结构）和前端的交互逻辑。这将成为前后端开发的“契约”。

2.  **数据模型与持久化 (Data Modeling & Persistence)**
    *   **目标**: 如果新功能需要存储数据，则定义相应的数据结构。
    *   **行动**:
        *   在 `backend/app/models/` 中定义数据库模型。
        *   在 `backend/app/schemas/` 中定义 Pydantic 数据验证模型。
        *   使用 Alembic 创建和执行数据库迁移脚本。

3.  **后端服务层实现 (Backend Service Implementation)**
    *   **目标**: 实现功能的业务逻辑。
    *   **行动**:
        *   在 `backend/app/service/` 目录下创建或修改相应的服务文件（例如，为写作助手功能创建 `writing_service.py`）。
        *   利用第一阶段构建的 `RAGService`、组件工厂和抽象层来编排核心逻辑。
        *   遵循依赖注入原则，确保服务是无状态且易于测试的。

4.  **API路由层接入 (API Routing)**
    *   **目标**: 将后端服务连接到HTTP接口。
    *   **行动**: 在 `backend/app/router/` 目录下创建新的路由文件，使用 FastAPI 的 `Depends` 将服务注入到API端点中，保持路由层的“轻”和“薄”。

5.  **前端组件与集成 (Frontend Implementation)**
    *   **目标**: 构建用户界面并与后端API联调。
    *   **行动**:
...
*   **任务 2.1: 智能化的在线文献获取与知识库构建 (Intelligent Online Paper Fetching & Knowledge Base Construction)**
    *   **第一部分：需求分析 (Part 1: Requirement Analysis)**
        ...
        *   **功能性需求 (Functional Requirements):**
            ...
            *   **FR4 - 质量筛选 (Quality Filtering)**:
                ...
                *   **务实方案 (V1)**: 我们将采用“**数据获取 + 后处理辅助决策**”的策略。
                    ...
                    2.  **辅助决策**: 系统内置一份核心的高质量会议/期刊列表（如CCF-A/B）。在返回的检索结果中，如果论文的发表源匹配此列表，则进行**特殊高亮**或标记，从而辅助用户进行手动筛选。
            *   **FR5 - 人机协同筛选 (Human-in-the-loop Review)**: 检索完成后，向用户展示一个清晰的列表，包含论文标题、作者、年份、会议/期刊、摘要。用户可以手动勾选确认需要导入的论文。
        ...
        *   **非功能性需求 (Non-Functional Requirements):**
            *   **NFR1 - 响应性 (Responsiveness)**: 检索和下载过程不能阻塞UI。整个流程需要有明确的进度提示（如：正在检索、等待用户确认、正在后台下载 X/Y 篇...）。
            *   **NFR2 - 稳定性与容错 (Stability & Fault Tolerance)**: 后台下载任务应是稳定的。单篇论文的下载失败（如找不到PDF源）不能中断整个批次。任务结束后，应向用户提供一份清晰的处理结果报告（成功N篇，失败M篇及原因）。
...
*   **任务 2.2: 多模态内容解析 (Multi-modal Ingestion)**
...
*   **任务 2.3: 语义感知分块 (Semantic-Aware Chunking)**
...
*   **任务 2.4: 基础能力：多文档会话 (Multi-Document Session Support)**
...
*   **任务 2.5: <font color='red'>高级检索策略 (Advanced Retrieval Strategies)</font>**
...
*   **任务 2.6: 答案溯源与高亮 (Citation & Highlighting)**
...
*   **任务 2.7: 【亮点功能】<font color='red'>跨论文深度对比分析 (Cross-Paper Comparative Analysis)</font>**
...
*   **任务 2.8: 【亮点功能】<font color='red'>批判性问题生成 (Critical Question Generator)</font>**
...
*   **任务 2.9: <font color='red'>【核心技术：RL微调】自适应重排序模型 (Adaptive Reranker via RL)</font>**
