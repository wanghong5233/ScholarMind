...
const state = proxy({
  ...
  workspaces: [] as LatexAgentAPI.WorkspaceSummary[],
  workspaceId: '' as string,
  workspaceConfig: {} as Record<string, any>,
  fileTree: [] as LatexAgentAPI.FileNode[],
  files: {} as Record<string, FileBuffer>,
  activeFilePath: '' as string,
  openedFiles: [] as string[],
  chatMessages: [] as LatexChatMessage[],
  executionHistory: [] as LatexAgentAPI.AgentStep[],
  compileResult: null as LatexAgentAPI.CompileResult | null,
  agentStatus: {
...
const actions = {
  ...
  setWorkspaces(list: LatexAgentAPI.WorkspaceSummary[]) {
  ...
  setFileTree(tree: LatexAgentAPI.FileNode[]) {
    state.fileTree = tree
  },
  ...
  appendChatMessage(message: Omit<LatexChatMessage, 'id' | 'createdAt'> & { id?: string; createdAt?: number }) {
  ...
  setExecutionHistory(history: LatexAgentAPI.AgentStep[]) {
    state.executionHistory = history
  },
  setCompileResult(result: LatexAgentAPI.CompileResult | null) {
...
export const latexAgentState = state
export const latexAgentActions = actions
