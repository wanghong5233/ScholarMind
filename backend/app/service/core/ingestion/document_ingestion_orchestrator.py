...
class DocumentIngestionOrchestrator:
    ...
    def search_online(
        ...
        *,
        ...
        Args:
            ...
            providers (List[str] | None): Provider list override.
            rank_by (str | None): Ranking strategy.

        Returns:
            List[DocumentCreate]: Search results.
        """
        try:
            knowledgebase_service.get_kb_by_id(
                db=self.db, kb_id=kb_id, user_id=self.current_user.id
        ...
        try:
            return ingestion_service.search_online_papers(
                query=query,
                limit=limit,
                year=year,
                db=self.db,
    ...
    def add_online_documents(
        ...
        *,
        ...
        """Create ingestion job for online documents.

        Args:
            kb_id (int): Knowledge base id.
            documents (List[DocumentCreate]): Document payload.
            background_tasks (BackgroundTasks): Background task queue.
