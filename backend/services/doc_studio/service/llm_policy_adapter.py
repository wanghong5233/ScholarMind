...
class _FallbackResolver:
    ...
    def resolve(
        ...
        *,
        ...
        default_max_tokens = int(getattr(settings, "LLM_MAX_TOKENS", 3072) or 3072)
        max_tokens = int(override_max_output_tokens or default_max_tokens)
        temperature = float(override_temperature if override_temperature is not None else getattr(settings, "LLM_TEMPERATURE", 0.2))
        ...
        timeout_secs = float(override_timeout_secs if override_timeout_secs is not None else getattr(settings, "LLM_REQUEST_TIMEOUT", 75))
        return type(
            "ResolvedTaskPolicy",
            (),
            {
