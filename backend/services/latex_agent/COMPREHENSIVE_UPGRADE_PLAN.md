...
| 4. 错误处理与降级 | 🟡 部分完成 | LLM 推理降级 OK，配置/工具级 fallback 待补 |
| 5. 可观测性增强 | 🟡 部分完成 | Prometheus 指标齐全，但缺少用户反馈收集 & Dashboard |
| 6. 安全防护 | ✅ 已完成 | Prompt Injection/速率限制/ENV 管理已落地 |
| 7. 性能优化 | 🟡 部分完成 | 工作区缓存已加，增量 Diff/LLM Cache/更多并发待做 |
| 8. 测试覆盖 | 🟡 部分完成 | 意图/计划等单测已写，缺集成测试与更高覆盖率 |
| 9. 代码质量提升 | ⛔ 未启动 | BaseLLMTool/类型同步尚未做，如非必要可跳过 |
|10. 用户体验优化 | 🟡 部分完成 | 状态/警告展示 OK，友好错误 & 等待提示待补 |

> **短期高收益但仍缺的功能**（建议优先级从高到低）  
> 1. **用户反馈闭环 + Grafana 面板**（模块 5）—— 能直接量化 Agent 表现，展示价值最大。  
> 2. **增量 Diff / LLM 调用缓存**（模块 7）—— 大文档性能提升明显，易在面试中展示前后对比。  
> 3. **端到端集成测试**（模块 8）—— 证明系统稳定度，亦是招聘方关注点。  
...
**当前实现（简单关键词匹配）**：
```python
# backend/services/latex_agent/service/intent_classifier.py
...
```python
# backend/services/latex_agent/service/intent_classifier_v2.py
...
**配置文件**：
```json
// backend/services/latex_agent/configs/intent_rules.json
{
  "version": "2.0",
  "rules": [
    {
...
**集成到 AgentService**：
```python
# backend/services/latex_agent/service/agent_service.py
...
**当前实现（固定工具序列）**：
```python
# backend/services/latex_agent/service/plan_builder.py
...
```python
# backend/services/latex_agent/service/plan_builder_v2.py
...
**配置文件**：
```json
// backend/services/latex_agent/configs/plan_strategy.json
{
  "version": "2.0",
  "strategies": {
    "EDIT": {
...
```python
# backend/services/latex_agent/service/error_handler.py
...
```python
# backend/services/latex_agent/config_validator.py
...
### 模块 5：可观测性增强 🔴 高优先级 — 🟡 部分完成

> 当前进度：Prometheus 暴露意图/计划/工具/工作区缓存指标。  
> 待办（建议优先）：  
> - 前端加点赞/点踩并上报 `agent_user_feedback_total`。  
> - Grafana 模板 & TraceID 关联。  
> - 失败对话的告警规则。
...
```python
# backend/services/latex_agent/observability.py
...
```typescript
// frontend/src/pages/latex-editor/index.tsx
...
panels:
  ...
  - title: "Top 10 慢工具"
    expr: topk(10, avg(latex_agent_tool_duration_seconds_total) by (tool))
```
...
```python
# backend/services/latex_agent/security.py
...
```python
# backend/services/latex_agent/rate_limiter.py
...
```python
# backend/services/latex_agent/config.py
...
> 当前进度：WorkspaceContextCache + cache/scan 指标已上线，RAG 批量检索异步化。  
> 待办（可选）：  
> - 增量 Diff / 大文件截断展示。  
> - LLM 响应缓存或重复调用合并。  
...
```python
# backend/services/latex_agent/service/diff_generator.py
...
```python
# backend/services/latex_agent/service/tools/retrieval_tools.py
...
```python
# backend/services/latex_agent/service/llm_cache.py
...
```python
# backend/services/latex_agent/tests/test_intent_classifier_v2.py
...
```python
# backend/services/latex_agent/tests/test_agent_integration.py
...
```python
# backend/services/latex_agent/service/tools/base_llm_tool.py
...
datamodel-codegen \
  --input backend/services/latex_agent/models.py \
...
```mermaid
gantt
    title LaTeX Agent 完整升级计划（7-10 天）
...
frontend/src/
└─ pages/latex-editor/index.tsx  # 交互逻辑重构
```
