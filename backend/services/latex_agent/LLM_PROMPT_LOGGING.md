...
```bash
# 查看 latex_agent 服务的实时日志
docker compose logs latex_agent --tail=0 -f
```
...
docker compose logs latex_agent --tail=0 -f | grep "LLM"
```
...
```bash
# 保存到文件方便后续分析
docker compose logs latex_agent > latex_agent_logs.txt
```
...
如果你想看到更详细的 DEBUG 日志，可以在 `backend/services/latex_agent/main.py` 中修改：
...
修改 `backend/services/latex_agent/.env` 文件（如果有），或在 `config.py` 中设置：
...
```bash
docker compose logs latex_agent | grep "trace-1234567890"
```

### 2. 只看 Prompt 和响应（过滤其他噪音）

```bash
docker compose logs latex_agent --tail=0 -f | grep -E "(📤|📥|User Intent)"
```

### 3. 统计工具调用次数

```bash
docker compose logs latex_agent | grep "tool_name" | sort | uniq -c
```

### 4. 查看最近 10 次 Prompt

```bash
docker compose logs latex_agent | grep "📤 完整的 LLM Prompt" | tail -10
```
...
```bash
# 保存完整日志供后续分析
docker compose logs latex_agent > debug_$(date +%Y%m%d_%H%M%S).log
```
...
```bash
# 只记录关键信息，减少 I/O
docker compose logs latex_agent --tail=100 | grep -E "(ERROR|WARNING)"
```
...
- `backend/services/latex_agent/service/llm_client.py`：LLM 客户端（添加了日志）
- `backend/services/latex_agent/service/agent_service.py`：Agent 服务（构建 Observation）
- `frontend/src/pages/latex-editor/index.tsx`：前端（Prompt 替换逻辑）
