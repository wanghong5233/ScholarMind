...
LaTeX Agent 已全面升级，现在类似 Cursor 的智能 Agent：
...
```
http://localhost:3000/latex-editor
```
...
# 最近 50 行
docker compose logs latex_agent --tail=50
```

### 进入容器
```bash
docker compose exec latex_agent bash
...
### 测试 LLM 连接
```bash
docker compose exec latex_agent python -c "
import asyncio
...
### 日志位置
- **Agent 日志**: `docker compose logs latex_agent`
...
```bash
# 方法 1：热重启（推荐，不重新构建）
docker compose restart latex_agent
