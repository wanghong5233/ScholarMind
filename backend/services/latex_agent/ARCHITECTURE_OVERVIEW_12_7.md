# LaTeX Agent 架构概览（2025-12 最新版）

本文档记录目前 LaTeX Agent（后端服务 + 前端编辑器）的整体架构、组件划分以及调试要点，方便后续团队成员快速理解与扩展。
...
graph TB
    ...
    subgraph "后端 LaTeX Agent Service"
...
- `metrics.py`：累计 `latex_agent_tool_calls_total` 与 `latex_agent_tool_duration_seconds_total`。
...
## 3. 前端架构（`frontend/src/pages/latex-editor`）
...
2. **指标查看**
   ...
   - 关注 `latex_agent_tool_calls_total{tool=...,status=...}` 和耗时指标。
...
4. **测试命令**
   - 后端：`cd backend/services/latex_agent && pytest`
...
**联系方式**：
- 后端问题：检查 `backend/services/latex_agent/` 目录
